...
library G {
    ...
    function do_ecdsa_verify(
        G1Point memory commit,
        G1Point memory pk,
        uint256[2] memory sign
    ...
    function isinf(G1Point memory p) internal pure returns (bool) {
        if (p.X == 0 && p.Y == 0) {
            return true;
        }
        return false;
    ...
    function g2add(
        G2Point memory p1,
        G2Point memory p2
    ) internal view returns (G2Point memory r) {
